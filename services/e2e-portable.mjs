#!/usr/bin/env node
/**
 * Qually E2E Test — Portable Version (ESM)
 *   npm install @mysten/sui @mysten/walrus axios dotenv
 *   node e2e-portable.mjs
 */
...
function getKeyPairRaw() {
  ...
  if (!b64) throw new Error('Set TEST_PRIVATE_KEY in .env');
  if (b64.startsWith('0x')) return Uint8Array.from(Buffer.from(b64.slice(2), 'hex')).slice(0, 32);
  if (/^[0-9a-fA-F]{64}$/.test(b64)) return Uint8Array.from(Buffer.from(b64, 'hex')).slice(0, 32);
...
try {
  const eventResp = await axios.post(
    process.env.TATUM_GATEWAY_URL || 'https://sui-testnet.gateway.tatum.io',
    {
      ...
      params: [{ MoveModule: { package: PACKAGE_ID, module: 'bounty' } }, null, null, { descending: true }],
    },
  ...
  if (events.length > 0) console.log('  Latest event type:', events[0]?.type?.split('::').pop());
